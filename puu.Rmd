...
#plot(puu,controls = ctree_control(maxsurrogate = 1))

#plot(puu, type="simple")

#plot(puu, terminal_panel)
#plot(puu, terminal_panel = node_terminal)
plot(puu,         
  inner_panel=node_inner(puu,
       abbreviate = FALSE,            
       pval = F, 
       id = FALSE),
     terminal_panel=node_barplot(puu,id=F,beside=T,ylines=1.2,gap=0,ymax=1))
     #terminal_panel=NULL
     #terminal_panel=node_terminal(puu, 
     #  abbreviate = TRUE,
     #  digits = 1,                   # few digits on numbers
     #  fill = c("white"),            # make box white not grey
     #  id = FALSE)

#plot(puu, tp_args = list(id = FALSE))
